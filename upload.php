...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container { max-width: 500px; width: 100%; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); padding: 40px; border-radius: 20px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); border: 1px solid rgba(255, 255, 255, 0.2); }
        ...
        .subtitle { color: #718096; font-size: 16px; font-weight: 400; }
        .file-info { background: #f7fafc; border: 2px dashed #cbd5e0; border-radius: 12px; padding: 20px; text-align: center; margin: 20px 0; transition: all 0.3s ease; }
        ...
        .file-input-label { display: inline-block; padding: 12px 24px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 8px; cursor: pointer; font-weight: 500; transition: all 0.3s ease; border: none; font-size: 16px; }
        ...
        button { background: linear-gradient(135deg, #48bb78 0%, #38a169 100%); color: white; padding: 15px 30px; border: none; border-radius: 10px; cursor: pointer; margin-top: 25px; font-size: 16px; font-weight: 600; transition: all 0.3s ease; width: 100%; position: relative; overflow: hidden; }
...
<body>
    <div class="container">
        ...
        <form id="uploadForm" action="uploadimg.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="file-info" id="fileInfo">
                ...
                <input type="file" id="iconFile" name="iconFile" accept=".png, .jpg, .jpeg" required>
    ...
    <script>
        ...
        function updateFileInfo(file) {
            ...
            const fd = document.getElementById('fileDetails');
            if (file) {
        ...
        document.getElementById('iconFile').addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                if (!fn.endsWith('.png') && !fn.endsWith('.jpg') && !fn.endsWith('.jpeg')) { showMessage('Solo se permiten PNG, JPG o JPEG', 'error'); e.target.value = ''; updateFileInfo(null); return; }
        ...
        document.getElementById('uploadForm').addEventListener('submit', function(e) {
            ...
            const fn = file.name.toLowerCase();
            if (!fn.endsWith('.png') && !fn.endsWith('.jpg') && !fn.endsWith('.jpeg')) { e.preventDefault(); showMessage('Solo PNG, JPG o JPEG', 'error'); return; }
